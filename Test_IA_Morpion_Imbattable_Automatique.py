	...
	while exemple == plateau :
		if symbole == "O" :
			...
			if plateau == [[1,2,3],[4,5,6],[7,8,"O"]] :
				choix = 5
				break
			if plateau == [["O",2,3],[4,"X",6],[7,8,9]] or plateau == [[1,2,"O"],[4,"X",6],[7,8,9]] :
				choix = 8
				break
			if plateau == [[1,"O",3],[4,"X",6],[7,8,9]] or plateau == [[1,2,3],["O","X",6],[7,8,9]] :
				choix = 9
				break
			if plateau == [[1,2,3],[4,"X","O"],[7,8,9]] or plateau == [[1,2,3],[4,"X",6],[7,"O",9]] :
				choix = 1
				break
			if plateau == [[1,2,3],[4,"X",6],["O",8,9]] or plateau == [[1,2,3],[4,"X",6],[7,8,"O"]] :
				choix = 2
				break
...
def morpion() :
	...
	# Mettez des # avant les 3 apostrophes des lignes 778 et 780
	# Enlevez les # des lignes 784 et 786
	#'''
	symbole = "O"
	#'''
	...
	# Mettez des # avant les 3 apostrophes des lignes 784 et 786
	# Enlevez les # des lignes 778 et 780
	'''
	symbole = "X"
	'''	
	# Si vous voulez essayer la partie où le joueur commence et si les instructions suivantes n'ont pas déjà été effectuées :
	# Mettez des # avant les 3 apostrophes des lignes 790 et 866
	# Enlevez les # des lignes 870 et 945
	#'''	
	...
	#'''
	# Si vous voulez essayer la partie où le joueur commence et si les instructions suivantes n'ont pas déjà été effectuées :
	# Mettez des # avant les 3 apostrophes des lignes 870 et 945
	# Enlevez les # des lignes 790 et 866
	'''
	premier_joueur = "IA"
	joueur = "IA"
									...
									if plateau != plateau_precedent :
										...
										for d in [1,2,3,4,5,6,7,8,9] :
											...
											if plateau != plateau_precedent :
												plateau_precedent = mettre_valeur(joueur,premier_joueur,symbole,plateau_precedent,choix8)
												joueur = changer(joueur)
												choix9 = remplir(plateau,symbole,joueur,premier_joueur)
												plateau = traduction(joueur,premier_joueur,symbole,plateau,plateau_precedent,choix9)
												joueur = changer(joueur)
												plateau = enlever(choix8,plateau)
												plateau_precedent = enlever(choix8,plateau_precedent)			
