...
def estPossible(plateau,joueur,premier_joueur,symbole,ligne,colonne,choix) :
	...
	if joueur == "IA" :
		if plateau[ligne][colonne] == "O" or plateau[ligne][colonne] == "X" :
			return False
		obligation(plateau,symbole,joueur,premier_joueur,reponse)
...
def verification_demande(choix,plateau,joueur,premier_joueur,symbole,reponse) :
...
def obligation(plateau,symbole,joueur,premier_joueur,reponse) :
...
def decision(plateau,joueur,premier_joueur,symbole,reponse) :
	...
	colonne = coordonnees[1]
	while erreur == 1 :
		if estPossible(plateau,joueur,premier_joueur,symbole,ligne,colonne,choix,reponse) == False :
			choix = randint(1,9)
			coordonnees = traduction(choix)
			ligne = coordonnees[0]
			colonne = coordonnees[1]
...
def morpion() :
	...
	reponse = "a"
	symbole = "a"
	print("IA morpion inattentive à l'attaque")
	print("")
	...
	while estVictorieux(plateau) != True :
		...
		choix = verification_demande(choix,plateau,joueur,premier_joueur,symbole,reponse)
		if reponse == "O" :
			plateau = decision(plateau,joueur,premier_joueur,symbole,reponse)
